...
- [x] 3.1 `routes/blog/[slug].tsx` を実装し、公開記事のMDX本文がレンダリングされることをブラウザで確認する
- [x] 3.2 詳細ページにタイトル・日付・タグ（存在する場合のみ）を表示する
- [x] 3.3 未知のスラッグおよびdraft記事のスラッグへのアクセスに対して404を返すことを確認する

## 4. 記事一覧ページ (`blog-post-list`)

- [x] 4.1 `routes/index.tsx` を実装し、公開対象の全記事をタイトル・日付・タグ・description付きで一覧表示する
- [x] 4.2 一覧を `date` の降順でソートし、日付が異なる複数記事を用意して並び順を確認する
- [x] 4.3 `draft: true` の記事が一覧に表示されないことを確認する
- [x] 4.4 一覧ページの各記事項目から対応する詳細ページ (`/blog/<slug>`) へのリンクを実装する

## 5. 静的サイト生成 (`static-site-generation`)

- [x] 5.1 `vite.config.ts` の `nitro()` プラグイン設定に `static: true`（全ルートをcrawlLinksでプリレンダー）を追加する
- [x] 5.2 公開記事を複数件・draft記事を1件用意した状態で `bun run build` を実行し、ビルド成果物に公開記事ページと一覧ページの静的HTMLが含まれることを確認する
- [x] 5.3 同じビルド成果物にdraft記事に対応する静的HTMLが含まれないことを確認する
